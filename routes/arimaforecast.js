...
async function setDataToCSV(res){
    ...
    dateExperimentstart.setDate(dateExperimentstart.getDate() + 2);
    dateExperimentend.setDate(dateExperimentend.getDate() + 2);    
    ...
    // console.log(process.env.HOMEBREW_PREFIX);
                      ...
                      var strDate = tpd.getFullYear()+"-"+(tpd.getMonth()+1)+"-"+tpd.getDate();           
                    
                    //   listdataraw.push({date: strDate, totaldata: leghtdataraw, filtereddata: countfilteredData });
                      listdataraw.push(countfilteredData);
                      dateraw.push(""+strDate);
        ...
        let fs = require('fs');
        let file = fs.createWriteStream("/home/rafif/TA/myappal/routes/gw.csv");
        var datacsv = [];
        for(let z = 0; z < listdataraw.length;z++){
            datacsv.push([dateraw[z],listdataraw[z]]);
            file.write('"'+dateraw[z]+'",'+listdataraw[z]+'\n');
        }
        file.end();
    // res.json(datacsv);
}
...
async function getForecast(res){
    ...
    let pyshell = spawn('python', ['/home/rafif/TA/myappal/routes/arima.py']);
    var stdoutdata = [];
